...
<template>
  <div class="h-screen bg-[#0a0f1a] relative overflow-hidden grid-bg">
    ...
    <div class="relative z-10 h-full flex flex-col items-center justify-center px-4 py-3 sm:py-6 lg:py-8">

      <!-- Logo -->
      <div class="mb-4 sm:mb-6 text-center">
        <div class="inline-flex items-center gap-2 sm:gap-3">
          <div class="relative">
            <div class="w-10 h-10 sm:w-12 sm:h-12 lg:w-14 lg:h-14 rounded-xl bg-gradient-to-br from-sky-500 to-cyan-600 flex items-center justify-center animate-pulse-glow overflow-hidden">
              <img v-if="logoUrl" :src="logoUrl" :alt="companyName" class="w-full h-full object-cover" />
              <Cloud v-else class="w-5 h-5 sm:w-7 sm:h-7 lg:w-8 lg:h-8 text-white" />
            </div>
            <div class="absolute inset-0 rounded-xl bg-gradient-to-br from-sky-400 to-cyan-500 blur-xl opacity-50" />
          </div>
          <div class="text-left">
            <h1 class="text-lg sm:text-xl lg:text-2xl font-bold text-white tracking-tight">{{ companyName }}</h1>
            <p class="text-[10px] sm:text-xs text-sky-400 font-medium tracking-wider uppercase">{{ companyTagline }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="w-full max-w-2xl">
        <div class="glass rounded-2xl sm:rounded-3xl p-5 sm:p-7 lg:p-10 text-center relative overflow-hidden">
          ...
          <!-- Heading -->
          <h2 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-2 sm:mb-3">
            Segera <span class="gradient-text">Hadir</span>
          </h2>

          <p class="text-gray-400 text-sm sm:text-base mb-5 sm:mb-6 max-w-md mx-auto leading-relaxed">
            Kami sedang membangun platform terbaik untuk Anda. Nantikan peluncuran kami!
          </p>

          <!-- Progress Steps -->
          <div class="flex items-center justify-center gap-0 mb-5 sm:mb-7 max-w-sm mx-auto">
            <template v-for="(step, i) in progressSteps" :key="step.label">
              <div class="flex flex-col items-center gap-1.5">
                ...
                <span :class="['text-[9px] sm:text-[10px] font-medium', step.done ? 'text-sky-400' : 'text-gray-500']">
                  {{ step.label }}
                </span>
              </div>
              <div
                v-if="i < progressSteps.length - 1"
                :class="['flex-1 h-0.5 mb-4 sm:mb-5 mx-1 sm:mx-2 rounded-full transition-all', step.done && progressSteps[i+1]?.done ? 'bg-gradient-to-r from-sky-500 to-cyan-500' : step.done ? 'progress-bar-animated' : 'bg-white/10']"
              />
            </template>
          </div>
          ...
          <div class="grid grid-cols-4 gap-2 sm:gap-3 mb-5 sm:mb-6">
            <div
              ...
              :key="service.label"
              class="group p-2 sm:p-3 rounded-lg sm:rounded-xl bg-white/5 border border-white/10 hover:border-sky-500/30 hover:bg-white/10 transition-all duration-300"
            >
              <div :class="['w-7 h-7 sm:w-9 sm:h-9 rounded-lg bg-gradient-to-br flex items-center justify-center mx-auto mb-1 sm:mb-2 group-hover:scale-110 transition-transform duration-300', service.color]">
                <component :is="service.icon" class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
              </div>
              <span class="text-[9px] sm:text-[10px] lg:text-xs text-gray-400 font-medium leading-tight">{{ service.label }}</span>
            </div>
          </div>

          <!-- Contact -->
          <div class="border-t border-white/10 pt-4 sm:pt-5">
            <p class="text-gray-500 text-[10px] sm:text-xs mb-2 sm:mb-3">Hubungi kami</p>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 mb-3">
              <a :href="`mailto:${supportEmail}`" class="flex items-center gap-1.5 px-3 py-1.5 sm:py-2 rounded-lg bg-sky-500/10 border border-sky-500/20 text-sky-400 hover:bg-sky-500/20 transition-colors">
                <Mail class="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                <span class="text-[10px] sm:text-xs font-medium">{{ supportEmail }}</span>
              </a>
              <a :href="`tel:${supportPhoneRaw}`" class="flex items-center gap-1.5 px-3 py-1.5 sm:py-2 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 hover:bg-cyan-500/20 transition-colors">
                <Phone class="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                <span class="text-[10px] sm:text-xs font-medium">{{ supportPhone }}</span>
              </a>
            </div>

            <div v-if="socialLinks.length || showWhatsapp" class="flex items-center justify-center gap-2">
              <a
                v-for="social in socialLinks"
                :key="social.label"
                :href="social.href"
                class="w-7 h-7 sm:w-9 sm:h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-sky-400 hover:border-sky-500/30 hover:bg-sky-500/10 transition-all duration-300"
              ...
              <a
                ...
                class="w-7 h-7 sm:w-9 sm:h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-green-400 hover:border-green-500/30 hover:bg-green-500/10 transition-all duration-300"
                aria-label="WhatsApp" target="_blank" rel="noopener noreferrer"
              >
                <svg viewBox="0 0 24 24" class="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="currentColor" aria-hidden="true">
      ...
      <!-- Footer -->
      <div class="mt-3 sm:mt-5 lg:mt-6 text-center">
        <p class="text-gray-600 text-[10px] sm:text-xs">&copy; {{ currentYear }} {{ companyFullName }}. All rights reserved.</p>
        <p class="text-gray-700 text-[10px] sm:text-xs mt-0.5">{{ companyDescriptor }}</p>
      </div>
    </div>
  </div>
</template>